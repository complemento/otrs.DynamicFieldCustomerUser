...
package var::packagesetup::DynamicFieldCustomerUser;
...
sub new {
    ...
    #$Self->{SysConfigObject}->SettingReset(Name => 'ZZZAAuto.pm');
    my @ZZZFiles = (
        'ZZZAAuto.pm',
        'ZZZAuto.pm',
    );
...
sub _DynamicFieldsDelete {
    ...
    for my $DynamicField ( @{$DynamicFieldList} ) {
        next DYNAMICFIELD if (
            $DynamicField->{FieldType} ne 'CustomerUserReference'
        );
...
sub _MigrateConfigBefore5 {
    ...
    for my $DynamicFieldID ( @{$List} ) {
        ...
        next DYNAMICFIELD if (
            $DynamicField->{FieldType} ne 'CustomerUserReference'
            && $DynamicField->{FieldType} ne 'CustomerUserReferenceArray'
        );
        ...
        if ($DynamicField->{FieldType} eq 'CustomerUserReference') {
            if ( !$DynamicField->{Config}->{MaxArraySize} ) {
                $DynamicField->{Config}->{MaxArraySize} = 1;
            }
        }
        if ($DynamicField->{FieldType} eq 'CustomerUserReferenceArray') {
            $DynamicField->{FieldType} = 'CustomerUserReference';
        }
